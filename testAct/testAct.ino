...
Here the variables  u,a,b,c,d are used to remember the number of fruits of the Grade : Undefined,A,B,C,D detected.
                    a2,b2,c2,d2 are for setting the sorter back to its stable position after current fruit is sorted.
...
void loop() {
    if((Serial.available()>0) && (analogRead(A0)>450))
    {
        Serial.println("Entering time into Queue");
        while((analogRead(A0)>450))
          while((millis()%10!=0))
            Serial.println(analogRead(A0));//To wait until the fruit has crossed the IR sensor
